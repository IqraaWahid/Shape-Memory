 ...
 * Class Code: ICS4U
 * Date: 11/05/2023
 * Purpose: To create the ImageDisplay class of the game.
 */
...
public class ImageDisplay extends JFrame {
    ...
    public ImageDisplay() {
        ...
        Thread task = new Thread(() -> {//starts thread
            try {
                Thread.sleep(DISAPPEAR_TIME_SECONDS * 1000);//15*1000=15000 milliseconds (hides screen in 15000 milliseconds)
                setVisible(false); // set visible false for window (close window)
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        });
        task.start(); // thread call to start the timer for 16 seconds
     ...
     * Pre: none
     * Post: none
     * Purpose: Loads the shapes images
     */
     ...
     * Pre: none
     * Post: none
     * Purpose: Displays the next shape image
     */
     ...
     * Pre: none
     * Post: none
     * Purpose: Hides the previous displayed shape image
     */
